...
test-role:
	$(COMPOSE) exec -T postgres psql -U cip -d cip -v ON_ERROR_STOP=1 		-f /dev/stdin < scripts/create_test_role.sql

# -rs prints the reason for every skip. Without it a run that touched nothing reports "49
...
test-integration:
	CIP_RUN_INTEGRATION=1 	CIP_POSTGRES__HOST=localhost CIP_POSTGRES__PORT=5432 CIP_POSTGRES__DATABASE=cip 	CIP_POSTGRES__USER=cip_app CIP_POSTGRES__PASSWORD=ci_app 	CIP_REDIS_URL=redis://localhost:6379/0 	CIP_MONGO__URI=mongodb://cip:devpassword@localhost:27017/cip?authSource=admin 	CIP_NEO4J__URI=bolt://localhost:7687 CIP_NEO4J__USER=neo4j CIP_NEO4J__PASSWORD=devpassword 	CIP_EVENTS_BROKER_URL=localhost:9092 	$(PY) -m pytest -q -rs -m integration --integration-min=50
